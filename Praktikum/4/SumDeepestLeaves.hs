...
deepest :: (BinTree Int) -> Int
deepest p = 
    if (isTreeEmpty p) then 0
    else if (isOneElmt p) then 1
    else 1 + max (deepest (left p)) (deepest (right p))

sumDeepestLeaves :: (BinTree Int) -> Int
sumDeepestLeaves p =
    -- if isTreeEmpty p then 0
    -- else if (isOneElmt p) then (akar p) + (sumDeepestLeaves (left p)) + (sumDeepestLeaves (right p))
    -- else (sumDeepestLeaves (left p)) + (sumDeepestLeaves (right p))

    if (isTreeEmpty p) then 0
    else if (isOneElmt p) then (akar p)
    else
        let dl = deepest (left p)
            dr = deepest (right p)
            in 
                if dl > dr then sumDeepestLeaves (left p)
                else if dr > dl then sumDeepestLeaves (right p)
                else (sumDeepestLeaves (left p)) + (sumDeepestLeaves (right p))
