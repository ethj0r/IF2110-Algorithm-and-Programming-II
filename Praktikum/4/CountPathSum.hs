...
countPathSum p target = 
    if (isTreeEmpty p) then 0
    else
        if (isTreeEmpty (left p) && isTreeEmpty (right p)) then
            if (akar p == target) then 1 else 0
        else
            countPathSum (left p) (target - (akar p)) + countPathSum (right p) (target - (akar p))
{-
Contoh 1:
Pohon:
         5
