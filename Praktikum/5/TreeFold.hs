...
treeFoldPost f b t =
    if (isTreeEmpty t) then b
    else
        let leftAcc = treeFoldPost f b (left t)
            rightAcc = treeFoldPost f b (right t)
            in
                f leftAcc rightAcc (akar t)
...
treeFoldPre f b t = 
    if (isTreeEmpty t) then b
        ...
        let leftAcc = treeFoldPre f b (left t)
            rightAcc = treeFoldPre f b (right t)
            in
                f (akar t) leftAcc rightAcc
...
treeFoldIn f b t =
    if (isTreeEmpty t) then b
        ...
        let leftAcc = treeFoldIn f b (left t)
            rightAcc = treeFoldIn f b (right t)
            in
                f leftAcc (akar t) rightAcc