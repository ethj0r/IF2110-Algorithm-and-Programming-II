...
editAtIndex l i x =
    if null l then
        ...
        if i == 0 then
            ...
            konso (head l) (editAtIndex(tail l)(i-1)x)
...
editMatrixElement m row col val =
    if null m then
        ...
        if row == 0 then
            konsoMatrix(editAtIndex(head m) (col) (val)) (tail m)
        else 
            konsoMatrix (head m) (editMatrixElement (tail m) (row-1) col val)
...
getMatrixElement m row col =
    if null m then
        ...
        if row == 0 then
            ...
            getMatrixElement (tail m) (row-1) col
